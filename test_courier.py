...
class TestCourier:
    ...
    def test_create_courier_success(self):
        ...
        response = requests.post(f"{BASE_URL}{COURIER_REGISTER}", data=data)
        assert response.status_code == 201
        assert response.json() == {"ok": True}

    def test_create_duplicate_courier(self):
        data = self.generate_courier_data()
        resp1 = requests.post(f"{BASE_URL}{COURIER_REGISTER}", data=data)
        assert resp1.status_code == 201

        # Попытка создать такого же курьера
        resp2 = requests.post(f"{BASE_URL}{COURIER_REGISTER}", data=data)
        # В зависимости от API — ожидаем ошибку (например, 409 или другой статус)
    ...
    def test_create_courier_missing_fields(self):
        ...
        for field in ["login", "password", "firstName"]:
            ...
            del data[field]
            response = requests.post(f"{BASE_URL}{COURIER_REGISTER}", data=data)
            # Ожидаем ошибку (например, 400 или 409)
            assert response.status_code != 201
